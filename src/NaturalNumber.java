...
public class NaturalNumber  {
	...
	public NaturalNumber plus( NaturalNumber  second) throws Exception{
        ...
        int loopSize = firstClone.coefficients.size();
        int carry = 0;
		Integer firstCloneInt = new Integer(0);
		Integer secondCloneInt = new Integer(0);
		int sumTmp = 0;
        for (int i = 0; i < loopSize; i++) {
		    firstCloneInt = firstClone.coefficients.removeFirst();
		    secondCloneInt = secondClone.coefficients.removeFirst();
		    sumTmp = firstCloneInt.intValue() + secondCloneInt.intValue() + carry;
		    carry = 0;
		    if (sumTmp > firstClone.base) {
		        sumTmp = sumTmp - firstClone.base;
		        carry = 1;
            }
            sum.coefficients.addLast(sumTmp);
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		for (int i = 0; i < size; i++) {
		    product.plus(timesSingleDigit(firstClone, multiplicand.coefficients.removeFirst(),i));
        }
	...
	public NaturalNumber timesSingleDigit(NaturalNumber multiplaned, int timesBy, int numZeros) {
	    NaturalNumber output = new NaturalNumber(multiplaned.base);
	    int carry = 0;
	    int tmp = 0;
	    int size = multiplaned.coefficients.size();
	    for (int i = 0; i < numZeros; i++) {
	        output.coefficients.addLast(0);
        }
	    for (int i = 0; i < size; i++) {
	        tmp = (multiplaned.coefficients.removeFirst() * timesBy) + carry;
	        carry = 0;
	        carry = tmp / multiplaned.base;
	        tmp = tmp % multiplaned.base;
	        output.coefficients.addLast(tmp);
        }
        output.coefficients.addLast(carry);
	    return output;
    }
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		int loopSize = this.coefficients.size();
		int firstTmpInt = 0;
		int secondTmpInt = 0;
		int tmp = 0;
		int carry = 0;
		for (int i = 0; i < loopSize; i++) {
		    firstTmpInt = first.coefficients.removeFirst();
		    secondTmpInt = second.coefficients.removeFirst();
		    if (firstTmpInt < secondTmpInt && first.coefficients.getFirst() != 0) {
		        int tmp2 = first.coefficients.getFirst();
		        tmp2 = tmp2 - 1;
                first.coefficients.removeFirst();
                first.coefficients.addFirst(tmp2);
                firstTmpInt = firstTmpInt + first.base;

            }
            tmp = 0;
		    tmp = firstTmpInt - secondTmpInt;
